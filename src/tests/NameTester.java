...
public class NameTester {
	...
	public static void main(String[] args) {
		...
		/*
		 * Now create an if statement where we can check if the two Names are the same.
		 * We should always test for failure so we will pass in two Name objects we know
		...
		/*
		 * Test the nameSearch method, again test for failure first by passing in a Name
		 * object that is not in the list. Next pass in one that is in the list
		 */
	...
	public static boolean nameSearch(Name name, ArrayList<Name> names) {
		...
		/*
		 * There is two ways we can now check if the ArrayList contains the name object,
		 * one we iterate over the ArrayList and then use our equals method to test are
		 * the two objects the same and return true if it is found.
		 */
		for (Name temp : names) {
		...
		/*
		 * As we are working with an ArrayList the ArrayList class comes with some handy
		 * methods of its own. One of these is the contains method which will check to
		 * see if an object is stored within it.
		 */
		/*
